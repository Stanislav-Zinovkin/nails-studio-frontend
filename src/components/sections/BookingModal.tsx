...
export default function BookingModal({ isOpen, onClose, t }: any) {
    ...
    // State for visually hiding.
    const [shouldHideBooksy, setShouldHideBooksy] = useState(false);

    if (!isOpen) return null;

    // Validation: whether at least one field is filled in.
    ...
    const handleBlur = () => {
        // If the field is empty, when leaving it, we revert the button Booksy
        if (!isFormStarted) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        console.log("Data for Email and safety:", formData);
        alert(`Thak you, ${formData.name}! Confirmation has been sent to ${formData.email}`);
        onClose();
        // Default state after closing modal
    ...
    return (
        <div 
            className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-[#10069F]/15 backdrop-blur-md animate-fadeIn"
            onClick={onClose}
        >
            <div 
                className="bg-[#FDF5E6] w-full max-w-md rounded-[40px] p-8 md:p-12 relative shadow-2xl border border-[#10069F]/5"
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Name */}
                    <div className="group">
                        <label className="text-[9px] uppercase tracking-[0.2em] text-[#10069F]/40 ml-4 mb-1 block">Imię i Nazwisko</label>
                        <input 
                            required
                            onFocus={handleFocus}
                            onBlur={handleBlur}
                    ...
                    {/* Email input */}
                    <div className="group">
                        <label className="text-[9px] uppercase tracking-[0.2em] text-[#10069F]/40 ml-4 mb-1 block">Email</label>
                        <input 
                            required
                            onFocus={handleFocus}
                            onBlur={handleBlur}
                    ...
                    {/*Phone input*/}
                    <div className="group">
                        <label className="text-[9px] uppercase tracking-[0.2em] text-[#10069F]/40 ml-4 mb-1 block">Telefon</label>
                        <input 
                            required
                            onFocus={handleFocus}
                            onBlur={handleBlur}
                    ...
                    <button 
                        type="submit" 
                        className="w-full bg-[#10069F] text-white rounded-2xl py-5 uppercase tracking-[0.25em] text-[11px] font-bold hover:bg-[#0c0580] transition-all shadow-lg shadow-[#10069F]/20 mt-4"
                    >
                        Zapisz się
                    </button>
                </form>
                ...
                <div className={`transition-all duration-700 ease-in-out overflow-hidden ${shouldHideBooksy ? 'max-h-0 opacity-0 mt-0' : 'max-h-60 opacity-100 mt-8'}`}>
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <span className="text-[10px] text-[#10069F]/30 uppercase tracking-[0.3em]">lub</span>
                        <div className="h-[1px] bg-[#10069F]/10 flex-grow"></div>
                    </div>
